...
class Snake(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = gym.spaces.MultiDiscrete([2*side, 2*side, side+1, side+1, side+1])
        self.reward_range = (0, side*side - 3)

    def reset(self):
        self.snake = [(side//2 - 1, side//2), (side//2 - 2, side//2)]
        self.snake_head = (side//2, side//2)
        bonus = (np.random.randint(side), np.random.randint(side))
    ...
    def get_obs(self):
        dist_x = self.bonus[0]-self.snake_head[0]+side
        dist_y = self.bonus[1]-self.snake_head[1]+side
        ...
        for i in range(side+1):
            if (self.snake_head[1]-i)==-1 or (self.snake_head[0], self.snake_head[1]-i) in self.snake:
                ...
                break
        for i in range(side+1):
            if (self.snake_head[0]+i)==side or (self.snake_head[0]+i, self.snake_head[1]) in self.snake:
                dist_right = i
                break
        for i in range(side+1):
            if (self.snake_head[1]+i)==side or (self.snake_head[0], self.snake_head[1]+i) in self.snake:
                dist_down = i
                break
        if self.direction==1:
            d_l, d_f, d_r = dist_left, dist_up, dist_right
            d_parallel, d_perpend = dist_x, dist_y
        elif self.direction==2:
            d_l, d_f, d_r = dist_up, dist_right, dist_down
            d_parallel, d_perpend = dist_y, dist_x
        elif self.direction==3:
            d_l, d_f, d_r = dist_right, dist_down, dist_left
            d_parallel, d_perpend = -dist_x, -dist_y
        else:
            d_l, d_f, d_r = dist_down, dist_left, dist_up
            d_parallel, d_perpend = -dist_y, -dist_x

        return d_parallel, d_perpend, d_l, d_f, d_r


    def get_obs_raw(self):
        curr_obs = [0 for i in range(side*side)]
        curr_obs[self.snake_head[0]*side + self.snake_head[1]] = 1
        for snake_block in self.snake:
            curr_obs[snake_block[0] * side + snake_block[1]] = 2
        curr_obs[self.bonus[0]*side + self.bonus[1]] = 3
        return curr_obs
    ...
    def step(self, action):
        ...
        if new_sh == self.bonus:
            ...
            while self.bonus == self.snake_head or self.bonus in self.snake:
                self.bonus = (np.random.randint(side), np.random.randint(side))
    ...
    def render(self, mode='human'):
        ...
        obs = ''.join(obs)
        for i in range(side):
            psdline = obs[side*i:side*i+side]
            print(psdline)
