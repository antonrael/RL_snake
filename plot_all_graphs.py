...
nom_envs=['env_1','env_2']
envs = [snake_gym_env_1.Snake(), snake_gym_env_2.Snake()]
rl_algorithms = ["Q_learning","SARSA"]
explore_methods=["epsilon_greedy","vdbe_epsilon","softmax"]
X_global=[]
Y_global={}
for env in nom_envs:
    Y_global[env]={}
...
def main():
    ...
    for nom_env in nom_envs:
        ...
        for rl_algorithm in rl_algorithms:
            for explore_method in explore_methods:
                X_global=[]
                X_agent=[]
                Y_agent=[]
                print(f'Env : {n_env}, rl_algorithm : {rl_algorithm}, explore_method : {explore_method}')
                ...
                for i_episode in range(n_episode):
                    ...
                    if i_episode % 1000 == 0:
                        if verbose:
                            if explore_method == VDBE_EPSILON:
                                print("Episode: {0}\t Num_Steps: {1:>4}\tMean_Return:{2:>5.2f}\tTotal_Return: {3:>5.2f}\tMean_epsilon: {4:.3f}".format(i_episode, i_step,mean_return, total_return, np.mean(epsilon_table)),end='\r')
                            else :    
                                print("Episode: {0}\t Num_Steps: {1:>4}\tMean_Return:{2:>5.2f}\tTotal_Return: {3:>5.2f}\tEpsilon: {4:.3f}".format(i_episode, i_step,mean_return, total_return, epsilon),end='\r')
                ...
                np.save(f'ALL_RESULTS/q_saved_{rl_algorithm}_{explore_method}_{n_episode}_episodes_env_{n_env}.npy', q_flat)
                ...
                plt.legend()
                plt.savefig(f'ALL_RESULTS/graph_{rl_algorithm}_{explore_method}_{n_episode}_episodes_env_{n_env}.png')
                plt.clf()
                #Add to global graph
                Y_global[nom_env][rl_algorithm][explore_method]=Y_mean
    
    #draw the global graph
    plt.xlabel('Episodes')
    plt.ylabel('Score')
    plt.title(f'All results')
    for nom_env in nom_envs:
        if nom_env=='env_1':
            m='x'
        if nom_env=='env_2':
        ...
        for rl_algorithm in rl_algorithms:
            ...
            for explore_method in explore_methods:
                ...
                plt.plot(X_global,Y_global[nom_env][rl_algorithm][explore_method],color=c, marker=m, linestyle=l,markeredgewidth=2.0,label=f'{nom_env}/{rl_algorithm}/{explore_method}',markevery=10)
    plt.legend()
    plt.show()
    plt.savefig(f'ALL_RESULTS/global_graph.png')

    #ZOOM
    longueur=len(X_global)//2
    X_global=X_global[0:longueur]
    plt.xlabel('Episodes')
    plt.ylabel('Score')
    plt.title(f'All results')
    for nom_env in nom_envs:
        ...
        for rl_algorithm in rl_algorithms:
            ...
            for explore_method in explore_methods:
                ...
                plt.plot(X_global[0:longueur],Y_global[nom_env][rl_algorithm][explore_method][0:longueur],color=c, marker=m, linestyle=l,markeredgewidth=2.0,label=f'{nom_env}/{rl_algorithm}/{explore_method}',markevery=5)
    plt.legend()
    plt.show()
    plt.savefig(f'ALL_RESULTS/global_graph_zoom1.png')
    ...
    for nom_env in nom_envs:
        ...
        for rl_algorithm in rl_algorithms:
            ...
            for explore_method in explore_methods:
                ...
                plt.plot(X_global[0:longueur],Y_global[nom_env][rl_algorithm][explore_method][0:longueur],color=c, marker=m, linestyle=l,markeredgewidth=2.0,label=f'{nom_env}/{rl_algorithm}/{explore_method}',markevery=2)
    plt.legend()
    plt.show()
    plt.savefig(f'ALL_RESULTS/global_graph_zoom2.png')
